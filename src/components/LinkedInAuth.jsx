...
export default function LinkedInAuth() {
  ...
  const handleLinkedInCallback = async (code) => {
    ...
    try {
      ...
      console.log(`this is user toke ${token}\n user data ${user}`)
  ...
  return (
    <div className="flex flex-col">
      {loading ? (
        <p className="text-lg text-gray-600">Processing LinkedIn login...</p>
      ) : (
        <button
          onClick={handleLinkedInLogin}
          className="flex items-center space-x-3 bg-[#0077B5] text-white px-4 py-2 rounded-lg shadow hover:bg-[#005b8c] transition"
        >
          <svg
          ...
          <span className="px-16 sm:px-2">Continue with LinkedIn</span>
        </button>
      )}
    </div>
  );
